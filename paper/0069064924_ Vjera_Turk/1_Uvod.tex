...
Tradicionalni pristupi procjeni \gls{pom} kao izvor podataka koriste podatke prikupljene anketom stanovništva i/ili prebrojavanjem vozila na strateškim položajima u prometnoj infrastrukturi. 
\subsection{Ankete}
...
\subsection{Raspored stanica za prebrojavanje}
...
\begin{enumerate}
	...
	U tom iterativnom procesu prometni tokovi iz modificirane \gls{pom}-e ponovno se dodjeljuju prometnoj infrastrukturi (sve dok se ne postigne zadani broj iteracija ili zadovoljavajuæi stupanj sliènosti poèetne i nove \gls{pom}-e - detaljnije u sekciji \ref{valid}). 
...
radi naknadnog procesiranja \cite{Jelusic:2016.}. Vozilo  se kreæe prometnom infrastrukturom, postajuæi tako dijelom prometnog toka. Osnovni podatci koje osjetilo prikuplja su položaj, brzina i smjer kretanja vozila \cite{Jelusic:2016.}.
...
\subsection{Prostorna podjela - Voronoi teselacija}
...
\textbf{Gustoæa Voronoi æelija dobro je korelirana s gustoæom stanovništva te stoga bolju prostornu rezoluciju ima na podruèjima veæe gustoæe populacije \cite{Gundlegard:2016.}}. Poistovjeti li se položaj korisnika s položajem tornja bazne stanice, \textbf{mjerna nesigurnost odreðivanja položaja iznosi od približno stotinu metara u gusto naseljenom, do nekoliko kilometara u ruralnom podruèju \cite{Calabrese:2011.}}.  Problem predstavlja preklapanje podruèja pokrivanja u stvarnosti \cite{Gundlegard:2016.}. Zbog uravnoteženja optereæenja u mreži (eng. \textit{load balancing}) ureðaj mijenja baznu stanicu na koju je spojen iako sve vrijeme zadržava isti položaj.
...
U literaturi je korištena razlièita vremenska rezolucija (razluèivost) - podjela na vremenske periode $\tau$ od 15 min, 30 min \cite{Schneider:2013.}, 1 sat, 2 sata, 3 sata \cite{Filic:2016.}\cite{Chen:2019.}. \cite{Filic:2016.,Chen:2019.}
...
\subsection{Rekonstrukcija putanja}
...
\textbf{Odbacivanje korisnika s niskim stupnjem \textit{potpunosti}  zastupljeno je u istraživanjima razvoja \gls{pom}-a iz \gls{cdr} što dovodi do iskorištavanja tek vrlo malog dijela korisnika - tek 1.67\% i 0.45\% u nekim istraživanjima} \cite{Chen:2019.}.
...
\section{Vrednovanje suvremene polazišno-odredišne matrice}
...
\section{Grafièki oblik i  sliènost strukture
...
\subsection{Višerazinski indeks strukturalne sliènosti} 
...
\subsection{Osnovni višerazinski indeks strukturalne sliènosti}
...
gdje su $\alpha$, $\beta$ i $\gamma$ parametri relativne važnosti svake komponente. Za SSIM vrijedi sljedeæe:
\begin{subequations}
	\begin{align}
...
\subsection{Poboljšani višerazinski indeks strukturalne sliènosti} 
