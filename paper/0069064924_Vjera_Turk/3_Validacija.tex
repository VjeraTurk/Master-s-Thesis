...
\todo[inline]{Transkript: \textit{...linearna regresija (neka bude) u svakom sluèaju za referentni model s kojim  usporeðujete ostale.}}
% toènost samo jedan od indikatora
\section{Toènost \glsfirstplural{pom}}

%\textit{Glavni indikator validacije?!}
Toènost procijenjenih \gls{pom}-a gotovo uvijek se definira u odnosu na referentnu matricu (eng. \textit{grand truth matrix}) koja je dobivena  tradicionalnim postupcima (anketiranje i/ili prebrojavanje vozila). Statistièke mjere kvantiziraju razliku procijenjenih i “istinitih“ vrijednosti, ako su nam one poznate. 

Èesto se u literaturi (jednoznaèno) koriste pojmovi \textit{toènost}, \textit{pouzdanost} i \textit{kvaliteta}. Za njihovu procjenu koriste se mjere koje opisuju razinu sliènosti odnosno razlike (greška) s referentnom matricom. 
\textbf{Tradicionalni postupak validacije \gls{pom}-e svodi se na odreðivanje njene toènosti u odnosu na \textit{grand truth} matricu.}

%goodness of fit measure
    
\subsection{Metrike za procjenu toènosti \glstext{pom}}

Za procjenu kvalitete \gls{pom}-a dobivenih iskljuèivo anketranjem u radu \cite{Cools:2010.} korištena je mjera Mean Apsolute Percentage Error (MAPE), te je prikazano da se zadovoljavajuæa razina kvalitete takvih \gls{pom}-a postiže tek ako uzorak obuhvaæa 50\% populacije. Istaknuta je važnost korištenja dodatnih izvora za izradu \gls{pom}-a.
...
Travassoli u svome radu \cite{Travassoli:2016.} navodi nekoliko uobièajeno korištenih mjera - {$R^{2}$, Geoffrey E. Havers statistics (GEH),   Root Mean Squared Error percentage \%RMSE te uvodi novu mjeru  Eigenvalue-based measure (EBM) (temeljenu na svojstvenim vrijednostima matrica) i procjenjuje pouzdanost \gls{pom}-e dobivene iz sustava automatskog prikupljanja podataka u javnom prijevozu (autobus, vlak i trajekt). Spominje i Wasserstein metric, mjeru koja se razlikuje po tome da ne usporeðuje samo vrijednosti parova istih æelija (elementwise). 
