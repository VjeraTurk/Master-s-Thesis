...
\textbf{Tradicionalni postupak validacije \gls{pom}-e svodi se na odreðivanje njene toènosti u odnosu na \textit{grand truth} matricu} analizom sliènosti. \cite{Travassoli:2016.} Statistièke mjere kvantiziraju razliku procijenjenih i “istinitih“ vrijednosti, ako su nam one poznate.  Èesto se u literaturi (jednoznaèno) koriste pojmovi \textit{toènost}, \textit{pouzdanost} i \textit{kvaliteta}. Za njihovu procjenu koriste se mjere koje opisuju razinu sliènosti odnosno razlike (greška) s referentnom matricom. 
